 ...
 //goal is to understand object building, handling undefined when dealing with operations. error rules for if statements(undefined > null > condition...) multiple error handling sets, scope, and eventually 
 //authentication using an api which is used on ther server and client side. upadating api state..., setting end times for api credentials(delete), cron jobs on api data, understanding configuration settings of api's, listening for changes in api obj state, ect...
 //basic math operations, sockets, ect.. 
 // a broad understanding of how to program. how to write a program. the five steps to writing a program in any language...
 ...
 init = (function() {
     ...
     function setAssignments() {
         ...
         if (!value || value == null || value.length > 700 || value.length < 20) {
             errorObject.tasksAndDutiesErrorId = "Assignment text must be between 20 and 700 characters";
             return;
         }
         tasksAndDuties = {
     ...
     function setAgreements() {
         ...
         if (formCheckBoxes.length !== 14) {
             location.replace("initContract.html?nope=Dont mess with the document");
         };
     ...
     function setSignature() {
         ...
         if (!signature || signature == null || signature.length < 4) {
             errorObject.signatureErrorId = "Signature: Please enter more than four letters";
         }
         if (Object.keys(errorObject).length > 0) {
             return;
         }
     ...
     function setContact() {
         ...
         if (!email || email == null || email.length < 0 || !/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(email)) {
             errorObject.emailErrorId = "your email is formatted incorrectly";
         }
         if (!phone || phone == null || phone.length < 0 || !/^\d{10}$/.test(phone)) {
             errorObject.phoneErrorId = "your phone is formatted incorrectly";
         }
         if (Object.keys(errorObject).length > 0) {
             return;
         }
     ...
     function viewInvoiceSet() {
         if (!paymentInfo.invoiceDates || paymentInfo.invoiceDates == null) {
             return;
         } //check days as well
         ...
         $.ajax({
             ...
             success: function(result, status, xhr) {
                 if (typeof(result) == "string") {
                     ...
                     $('#myModal').modal('hide');
                     for (let i = 0; i < errorsLength.length; i++) {
                         errorsLength[i].innerHTML = "";
                     }
 ...
 countErrors = (function() {
     ...
     for (let i = 0; i < input.length; i++) {
         if (i == 1 || i == 2 || i == 4) {
             input[i].onchange = function(e) {
                 checkCurrent(e.target.value, i, e.target.nextElementSibling)
             }
         } else {
     ...
     function checkCurrent(value, index, targ) {
         ...
         if (count === Object.keys(errObject).length) {
